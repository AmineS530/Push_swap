...
/*   Updated: 2023/03/21 19:33:25 by asadik           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*gnl_substr(char *s, unsigned int start, size_t len, int freesub)
{
	char	*sub;
	size_t	i;
...
char	*gnl_find_line(char *alpha)
{
	...
	while (alpha[n_line.i] != '\0' && alpha[n_line.i] != '\n')
		n_line.i++;
	n_line.nextline = gnl_substr(alpha, 0, n_line.i + 1, 0);
...
char	*gnl_set_remainder(char *alpha)
{
	...
	while (alpha[n_line.i + n_line.owo] != '\0')
		n_line.owo++;
	alpha = gnl_substr(alpha, n_line.i, n_line.owo + n_line.i + 1, 42069);
	return (alpha);
}
...
char	*gnl_read_from_fd(int fd, char *alpha)
{
	...
	while (read_fd.bytes_read != 0)
	{
		...
		alpha = gnl_strjoin(alpha, read_fd.buffer);
		if (gnl_strchr(alpha, '\n'))
			break ;
	}
	free(read_fd.buffer);
